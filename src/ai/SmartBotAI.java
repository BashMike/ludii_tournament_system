...
public class SmartBotAI extends BotAI {
    protected int player = -1;

    public SmartBotAI(String id) {
        super(id);
    ...
    )
    {
        ...
        )
        {
            ...
            while (current != null) {
                ...
                for (int p = 1; p <= game.players().count(); ++p) { current.scoreSums[p] += utilities[p]; }
     ...
     *
     * Selected node (if it has 0 visits, it will be a newly-expanded node).
     */
    ...
    public static Move finalMoveSelection(final Node rootNode) {
        ...
        for (int i = 0; i < numChildren; ++i) {
            ...
            // this case implements random tie-breaking
            else if (visitCount == bestVisitCount && ThreadLocalRandom.current().nextInt() % ++numBestFound == 0) { bestChild = child; }
        }
    ...
    @Override
    public void initAI(final Game game, final int playerID) { this.player = playerID; }

    @Override
    public boolean supportsGame(final Game game) {
        if      (game.isStochasticGame())       { return false; }
        else if (!game.isAlternatingMoveGame()) { return false; }
        return true;
    }

    private static class Node {
        private final Node                parent;         // Our parent node
        private final Move                moveFromParent; // The move that led from parent to this node
        private final Context             context;        // This objects contains the game state for this node (this is why we don't support stochastic games)

        private int                       visitCount = 0; // Visit count for this node
        private final double[]            scoreSums;      // For every player, sum of utilities / scores backpropagated through this node
        private final List<Node>          children = new ArrayList<Node>(); // Child nodes
        private final FastArrayList<Move> unexpandedMoves; // List of moves for which we did not yet create a child node
